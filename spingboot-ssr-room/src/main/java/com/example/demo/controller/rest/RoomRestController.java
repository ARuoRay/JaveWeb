...
public class RoomRestController {
	...
	public ResponseEntity<ApiResponse<List<RoomDto>>>getRooms(){
		...
		String message=roomDtos.isEmpty()?"Room查無資料":"Room查詢多筆成功";
	...
	public ResponseEntity<ApiResponse<RoomDto>>addRoom(@RequestBody RoomDto roomDto){
		...
		return ResponseEntity.ok(ApiResponse.success("Room新增成功", roomDto));
	}
	//取得單筆
	@GetMapping("/{roomId}")
	...
	public ResponseEntity<ApiResponse<RoomDto>>updateRoom(@PathVariable Integer roomId,@RequestBody RoomDto roomDto){
		...
		return ResponseEntity.ok(ApiResponse.success("Room修改成功", roomDto));
	}
	// 刪除房間
	@DeleteMapping("/{roomId}")
	public ResponseEntity<ApiResponse<Integer>>deleteRoom(@PathVariable Integer roomId){
		roomService.deleteRoom(roomId);
		return ResponseEntity.ok(ApiResponse.success("Room刪除成功", roomId));
	}
	
	@ExceptionHandler(RoomException.class)
	public ResponseEntity<ApiResponse<Void>>handleRoomException(RoomException e){
